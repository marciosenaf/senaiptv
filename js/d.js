  ...
  if (typeof performance !== "undefined" && typeof Storage !== "undefined") {
    ...
    if (nav_type == 1) {
      ...
      if (
        wautime != null &&
        waucount != null &&
        Math.floor(Date.now() / 1e3) - wautime < 300
      ) {
        var count = Math.round(
          Math.random() * (waucount * 1.02 - waucount * 0.98) + waucount * 0.98
        );
...
function WAU_dynamic_request(key, color, size, async_index) {
  ...
  if (document.title)
    ttl = encodeURIComponent(
      document.title.substr(0, 80).replace(/(\?=)|(\/)/g, "")
    );
...
function WAU_lrd() {
  ...
  if (
    ...
    if (
      (is_dst && tzoffset >= -120 && tzoffset <= -60) ||
      (!is_dst && tzoffset >= -60 && tzoffset <= 0)
    ) {
      return false;
    }
  } else {
